...
from adjust_death_rate_constant import adjustment_graph as adj_graph
from evolve_model import evolve_mask_graph as evo_mask_graph
from evolve_model import evolve_social_graph as evo_social_graph
...
class GUI:
    ...
    def stepModel(self):
        if variables.evolve_complete or variables.adjustment_complete:

            
            if variables.evolve_complete:
                if variables.mask_evolve_complete:
                    self.closeSimulation()
                    self.make_mask_evolution_graph()
                    return
                
                elif variables.social_evolve_complete:
                    self.closeSimulation()
                    self.make_social_evolution_graph()
                    return
                
            elif variables.adjustment_complete:
                self.closeSimulation()
                self.make_adjustment_graph()
                return
        
        if self.currentStep >= con.maximum_steps or variables.run_ended == 1:
            variables.run_ended = 0
            variables.test_counter += 1
            self.resetModel()
            self.runEvent()
    ...
    def make_adjustment_graph(self):
        adj_graph()
        
    def make_mask_evolution_graph(self):
        evo_mask_graph()
        
    def make_social_evolution_graph(self):
        evo_social_graph()
